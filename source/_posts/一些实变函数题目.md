...
> 设 $A,B,C$ 是 $\mathbb{R}^n$ 中的点集，且有 $$m^*(A\triangle B)=m^*(B\triangle C)=0$$ 求证 $m^*(A\triangle C)=0$.
...
> 设 $E\subset\mathbb{R}^n$，若对于 $\forall~x\in E$，总存在开球 $B(x,\delta_x)$，使得 $m^*(E\cap B(x,\delta_x))=0$，试证明 $m^*(E)=0$.

**证明**：有条件，存在一个开球族 $\mathscr{B}$，s.t. $x\in E$，并且 $E=\bigcup\limits_{B_x\in\mathscr{B}}(E\cap B_x)$. 可以取适当的 $\delta_x$，使得 $B$ 中的开球两两不相交，此时 $\mathscr{B}$ 为 $\mathbb{R}^n$ 中互不相交的开集族，即 $\mathscr{B}$ 至多可数，且 $\forall~B_x\in\mathscr{B}$，总有 $m^*(B_x\cap E)=0$.
...
即有 $\forall~n\in\mathbb{N}$，均有 $\sum\limits_{k=1}^n m^*(B_k)\leqslant m^*\left(\bigcup\limits_{k=1}^\infty B_k\right)$，

故对上式左侧也取极限，有 $\sum\limits_{k=1}^\infty m^*(B_k)\leqslant m^*\left(\bigcup\limits_{k=1}^\infty B_k\right).$
